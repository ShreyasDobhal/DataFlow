...
public class InputParallelogram extends Shapes{
    private int allx[]=new int[4];
    private int ally[]=new int[4];

    public InputParallelogram(int x,int y,int width,int height){
        super.x = x;
        super.y = y;
        super.height = height;
        super.width = width;
    ...
    public void setLocationAndDraw(int x,int y){
        ...
        g.drawString("Input : ",x+width/4,y+height/2);
        //super.setLocationAndDraw(x, y);
        if(this.text != null){
            //System.out.println("text"+text);
            g.setColor(Color.WHITE);
            g.drawString("Input : "+this.text,x+width/4,y+height/2);
        }
